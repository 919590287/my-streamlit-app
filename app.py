...
def create_visualization_charts(
    ...
    # 修复：使用 prop 参数传递 FontProperties 对象
...
def main():
    ...
    st.markdown(
        ...
        **功能说明：**
        ...
        3. 👥 生成 households.csv 与 persons.csv  
        4. 🚌 生成 ActivitySim 风格的 tours.csv 和 trips.csv
        5. 🔧 生成 MATSim population.xml 文件
        6. ✅ DTD验证和数据质量检查
        7. 📊 生成可视化图表（修复中文显示和时间分布）
        8. 📥 下载所有结果文件
        """
    )
    ...
    with st.expander("2.1 家庭规模分布", expanded=False):
        ...
        p_h1 = col_h1.number_input("1人户", 0.0, 1.0, 0.30, 0.01)
        p_h2 = col_h2.number_input("2人户", 0.0, 1.0, 0.40, 0.01)
        p_h3 = col_h3.number_input("3人户", 0.0, 1.0, 0.20, 0.01)
        p_h4 = col_h4.number_input("4人户", 0.0, 1.0, 0.10, 0.01)
        p_h5p = col_h5.number_input("5人+", 0.0, 1.0, 0.00, 0.01)

    hhsize_dist = {"1": p_h1, "2": p_h2, "3": p_h3, "4": p_h4, "5+": p_h5p}

    with st.expander("2.2 收入分段", expanded=False):
        col_l1, col_l2 = st.columns(2)
        low_min = col_l1.number_input("低收入最小值", 0.0, 1e9, 0.0, 1000.0)
        low_max = col_l2.number_input("低收入最大值", 0.0, 1e9, 300000.0, 1000.0)

        col_m1, col_m2 = st.columns(2)
        mid_min = col_m1.number_input("中收入最小值", 0.0, 1e9, 300000.0, 1000.0)
        mid_max = col_m2.number_input("中收入最大值", 0.0, 1e9, 600000.0, 1000.0)

        col_hh1, col_hh2 = st.columns(2)
        high_min = col_hh1.number_input("高收入最小值", 0.0, 1e9, 600000.0, 1000.0)
        high_max = col_hh2.number_input("高收入最大值", 0.0, 1e9, 2000000.0, 1000.0)

        st.markdown("#### 收入段权重")
        col_w1, col_w2, col_w3 = st.columns(3)
        w_low = col_w1.number_input("低收入占比", 0.0, 1.0, 0.3, 0.01)
        w_mid = col_w2.number_input("中收入占比", 0.0, 1.0, 0.5, 0.01)
        w_high = col_w3.number_input("高收入占比", 0.0, 1.0, 0.2, 0.01)
    ...
    with st.expander("2.4 年龄结构与劳动/在学/驾照率", expanded=False):
        ...
        s_0_5 = col_a1.number_input("0-5岁", 0.0, 1.0, 0.05, 0.01)
        s_6_17 = col_a2.number_input("6-17岁", 0.0, 1.0, 0.15, 0.01)
        s_18_22 = col_a3.number_input("18-22岁", 0.0, 1.0, 0.10, 0.01)
        s_23_64 = col_a4.number_input("23-64岁", 0.0, 1.0, 0.55, 0.01)
        s_65p = col_a5.number_input("65+岁", 0.0, 1.0, 0.15, 0.01)
        ...
        wr_16_17 = col_w0.number_input("16-17岁", 0.0, 1.0, 0.05, 0.05)
        wr_18_22 = col_w1.number_input("18-22岁", 0.0, 1.0, 0.30, 0.05)
        wr_23_59 = col_w2.number_input("23-59岁", 0.0, 1.0, 0.80, 0.05)
        wr_60_64 = col_w3.number_input("60-64岁", 0.0, 1.0, 0.40, 0.05)
        wr_65p = col_w4.number_input("65+岁就业", 0.0, 1.0, 0.10, 0.05)
        ...
        sr_6_17 = col_s1.number_input("6-17岁在学率", 0.0, 1.0, 0.95, 0.05)
        sr_18_22 = col_s2.number_input("18-22岁在学率", 0.0, 1.0, 0.70, 0.05)

        student_rate_by_age = {"6-17": sr_6_17, "18-22": sr_18_22}

        st.markdown("#### 驾照率")
        col_lc1, col_lc2, col_lc3, col_lc4 = st.columns(4)
        lr_18_22 = col_lc1.number_input("18-22岁驾照", 0.0, 1.0, 0.50, 0.05)
        lr_23_59 = col_lc2.number_input("23-59岁驾照", 0.0, 1.0, 0.90, 0.05)
        lr_60_69 = col_lc3.number_input("60-69岁驾照", 0.0, 1.0, 0.70, 0.05)
        lr_70p = col_lc4.number_input("70+岁驾照", 0.0, 1.0, 0.40, 0.05)
    ...
    if st.button("🏠 生成 Households & Persons"):
    ...
    if st.session_state["hh_df"] is not None and st.session_state["persons_df"] is not None:
        ...
        st.dataframe(hh_df.head(20))

        st.markdown("#### persons 预览")
        st.dataframe(persons_df.head(20))
    ...
    with st.expander("Tour 生成参数", expanded=True):
        ...
        st.info("💡 提示：早上7:00 = 07:00:00，下午5:00 = 17:00:00")

        col_tw1, col_tw2 = st.columns(2)
        with col_tw1:
            work_start_early = time_input_hms("Work-最早出发", 420, "work_early")  # 07:00
            work_start_late = time_input_hms("Work-最晚出发", 540, "work_late")  # 09:00
            school_start_early = time_input_hms("School-最早出发", 390, "school_early")  # 06:30
            school_start_late = time_input_hms("School-最晚出发", 480, "school_late")  # 08:00

        with col_tw2:
            shop_start_early = time_input_hms("Shop-最早出发", 540, "shop_early")  # 09:00
            shop_start_late = time_input_hms("Shop-最晚出发", 1140, "shop_late")  # 19:00
            social_start_early = time_input_hms("Social-最早出发", 600, "social_early")  # 10:00
            social_start_late = time_input_hms("Social-最晚出发", 1200, "social_late")  # 20:00
    ...
    if "tours_df" in st.session_state and st.session_state["tours_df"] is not None:
        ...
        st.info("💡 修复说明：现在图表会显示实际的早晚高峰时段，包括去程和回程的时间分布")
    ...
    st.markdown(
        ...
        <p>ActivitySim/MATSim 人口与出行生成工具 v2.0</p>
        <p>✅ 已修复：中文字体显示、时间分布统计、早晚高峰可视化</p>
        </div>
        """,
